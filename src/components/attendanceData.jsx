...
const AttendanceData = () => {
    ...
    const query = state?.query || {
        studentId: "F/ND/34/30017",
        studentName: "Miracle Olasoyn",
                    ...
                    <div className="h-full w-full relative flex justify-around min-h-screen">
                        ...
                        <div className="relative z-10 w-[750px] mb-8">
                            ...
                            <div className="h-[120px] w-full mt-4 mb-4">
                                <p className="font-semibold font-roboto">Matric Number: <span className="font-light">{query.studentId}</span></p>
                                <p className="font-semibold font-roboto">Student Name: <span className="font-light">{query.studentName}</span></p>
                                <p className="font-semibold font-roboto">Attendance Starting Date: <span className="font-light">{query.startDate}</span></p>
                                <p className="font-semibold font-roboto">Attendance Ending Date: <span className="font-light">{query.endDate}</span></p>
                            </div>
                                ...
                                <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                    <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-700 dark:bg-opacity-75">
                                        <tr>
                                            ...
                                            <th scope="col" className="px-6 py-3">Attendance date</th>
                                    ...
                                    <tbody className="text-white">
                                        {attendanceData.length > 0 ? (
                                            attendanceData.map((record, index) => (
                                                <tr
                                                    key={`${record.studentId}-${record.attendanceDate}-${index}`}
                                            ...
                                            <tr className="odd:dark:bg-opacity-75 odd:dark:bg-[#1F3A1F] even:bg-white even:dark:bg-[#1F3A1F] dark:border-gray-700 border-gray-200">
